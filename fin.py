...
with open('newplayers.csv','r') as csvinput:
...
#os.rename("output.csv", "playersupdated.csv")
		...
		with open("output.csv", "w") as csvoutput:
			...
			for li in play:
				for key in li.keys():
					...
					#f.write("\n")
					for row in reader:
						if li["playerId"] == int(row[8]):
							row.append(li["goals"])
						else:
							row.append(0)
						all1.append(row)
			writer.writerows(all1)
		csvoutput.close()	
	csvinput.close()
	return x
	
match = records.filter(lambda x:len(x)!=12)
newey = match.filter(func_play_id)
newey.pprint()
...
def prof(x):
	with open('output.csv','r') as csvinput:
		with open('dummy.csv', 'a') as csvoutput:
			writer = csv.writer(csvoutput, lineterminator='\n')
			reader = csv.reader(csvinput)
			all2 = []
			row = next(reader)
			row.append("Fouls")
			row.append("Own Goals")
			row.append("Pass accuracy")
			row.append("Shot effectiveness")
			all2.append(row)
			for row in reader:
				if int(row[8]) == x[0]:
					row.append(x[1])
...
pass_shot = pass_accuracy.join(shot_effectiveness)
profi=foul_go.join(pass_shot)
profile=profi.map(lambda x:(x[0], x[1][0][0], x[1][0][1], x[1][1][0], x[1][1][1]))
final_prof = profile.filter(prof)
#final_prof.pprint()
